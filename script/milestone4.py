...
with tab6:
    ...
    if df.empty:
        ...
        avg_temp = df["temperature_celsius"].mean() if "temperature_celsius" in df.columns else None
        avg_humidity = df["humidity"].mean() if "humidity" in df.columns else None
        avg_pm25 = df["air_quality_PM2.5"].mean() if "air_quality_PM2.5" in df.columns else None
        avg_pm10 = df["air_quality_PM10"].mean() if "air_quality_PM10" in df.columns else None
        avg_wind = df["wind_kph"].mean() if "wind_kph" in df.columns else None

        insights = "### 🌍 Automated Climate Insights\n\n"

        # --- Temperature Insights ---
        if avg_temp is not None:
            if avg_temp > 0.7:
                insights += f"- 🔥 *High average temperature* detected ({avg_temp:.2f} normalized).\n"
            elif avg_temp < 0.3:
                insights += f"- 🧊 *Cooler temperature trend* ({avg_temp:.2f}).\n"
            else:
                insights += f"- 🌤 *Moderate average temperature* ({avg_temp:.2f}).\n"

        # --- Humidity Insights ---
        if avg_humidity is not None:
            if avg_humidity > 0.7:
                insights += f"- 💧 *High humidity levels* observed.\n"
            elif avg_humidity < 0.3:
                insights += f"- 🌵 *Low humidity*, possibly indicating dry conditions.\n"
            else:
                insights += f"- 🌫 *Moderate humidity* overall.\n"

        # --- Air Quality Insights ---
        if avg_pm25 is not None and avg_pm10 is not None:
            if avg_pm25 > 0.6 or avg_pm10 > 0.6:
                insights += f"- 😷 *Poor air quality* detected — PM2.5: {avg_pm25:.2f}, PM10: {avg_pm10:.2f}.\n"
            else:
                insights += f"- 🌱 *Healthy air quality levels* — PM2.5: {avg_pm25:.2f}, PM10: {avg_pm10:.2f}.\n"

        # --- Wind Insights ---
        if avg_wind is not None:
            if avg_wind > 0.6:
                insights += f"- 🌬 *Strong winds* observed on average.\n"
            else:
                insights += f"- 🍃 *Mild wind conditions*.\n"

        # --- Correlation Insights ---
        corr = df.select_dtypes(include=["float64", "int64"]).corr()
        if "temperature_celsius" in corr.columns and "humidity" in corr.columns:
            temp_hum_corr = corr.loc["temperature_celsius", "humidity"]
            if temp_hum_corr < -0.5:
                insights += "- 📉 *Strong negative correlation* between temperature and humidity.\n"
            elif temp_hum_corr > 0.5:
                insights += "- 📈 *Positive correlation* between temperature and humidity.\n"
            else:
                insights += "- ⚖ *Weak relationship* between temperature and humidity.\n"

        # --- Context Info (Country/Year) ---
        if "country" in df.columns:
            country_text = "All Countries" if 'selected_country' not in locals() or selected_country == "All" else selected_country
            insights = f"### 🗺 Insights for *{country_text}*\n\n" + insights
        ...
        st.sidebar.download_button(
            "📥 Download Insights",
            insights,
            file_name="climate_insights.txt"
        )